...
class DownloadoffloadDAO {
	...
	public $arrStartDate = '';
	public $arrEndDate = '';
	public $offloadStartDate = '';
	public $offloadEndDate = '';
	public $uploadStartDate = '';
	public $uploadEndDate = '';
	...
	public function __construct($dbConnection) {
		...
		$_SESSION['configType'] =  $_REQUEST['config'];	
		
		
		$_SESSION['dateFilter'] =  $_REQUEST['dateFilter'];
		if($_REQUEST['dateFilter']=='UploadDate'){
			$_SESSION['startDate'] =  $_REQUEST['uploadStartDate'];
			$_SESSION['endDate'] =  $_REQUEST['uploadEndDate'];
		}else if($_REQUEST['dateFilter']=='DepartureTime'){
			$_SESSION['startDate'] =  $_REQUEST['depStartDate'];
			$_SESSION['endDate'] =  $_REQUEST['depEndDate'];
		}else if($_REQUEST['dateFilter']=='ArrivalTime'){
			$_SESSION['startDate'] =  $_REQUEST['arrStartDate'];
			$_SESSION['endDate'] =  $_REQUEST['arrEndDate'];
		}else if($_REQUEST['dateFilter']=='OffloadDate'){
			$_SESSION['startDate'] =  $_REQUEST['offloadStartDate'];
			$_SESSION['endDate'] =  $_REQUEST['offloadEndDate'];
		}
		$_SESSION['status'] =  $_REQUEST['status'];
		$_SESSION['tailsign'] =  $_REQUEST['tailsign'];
		$_SESSION['depAirport'] =  $_REQUEST['depAirport'];		
		$_SESSION['arrAirport'] =  $_REQUEST['arrAirport'];
		$_SESSION['source'] =  $_REQUEST['source'];
		$_SESSION['failureReason'] =  $_REQUEST['failureReason'];
	...
	public function getPlatformsForAirline() {
		...
		if(isset($this->airlineId) && $this->airlineId != -1) {
		    $query .=  "where airlineID IN ($this->airlineId) ";
		}
		
		$query .= "order by platform";
		error_log("getPlatformForAirline Query: " . $query);
	...
	public function getConfigTypesForAirlineAndPlatform() {
	    $query = "select distinct Ac_Configuration as configType from aircrafts where 1=1 ";
		...
		if ($this->platform != '') {
			$query .= "and platform='$this->platform' ";//select distinct Ac_Configuration as configType from aircrafts where airlineID=$this->airlineId and Ac_Configuration <> '' order by Ac_Configuration";
		}

		$query .= "and Ac_Configuration <> '' order by Ac_Configuration";
		error_log("Config Type Query: " . $query);
		$result = mysqli_query ( $this->dbConnection, $query );
	...
	function getDepArrAirportList() {
	    error_log('inside getDepArrAirportList');
...
// 		$query = "select (SELECT group_concat(DISTINCT depAirport) FROM $mainDB.offloads_master WHERE 1=1 ";
...
// 		if ($this->tailsign != '') {
// 		    $query .= "AND tailsignFound='$this->tailsign' ";
// 		} else if(!$this->isNullOrEmptyString($this->platform) or !$this->isNullOrEmptyString($this->configType)) {
// 		    $tailsignString = "";
// 		    $tailsignArray = $this->getTailsignForAirlinePlatformConf();
// 		    foreach ($tailsignArray as $tailsign) {
// 		        $tailsignString .= "'" . $tailsign['tailsign'] . "',";
// 		    }
// 		    $tailsignString = rtrim($tailsignString, ",");
// 		    $query .= "AND tailsignFound in ($tailsignString) ";
// 		}
		
// 		$query .= "order by depAirport) as depAirportList, (SELECT group_concat(DISTINCT arrAirport) FROM $mainDB.offloads_master WHERE 1=1 ";
...
// 		$query .= "order by arrAirport) as arrAirportList";
		
		$query="SELECT group_concat(DISTINCT om.depAirport) as depAirportList, group_concat(DISTINCT om.arrAirport) as arrAirportList FROM $mainDB.offloads_master om WHERE 1=1 ";
		
		if(isset($this->airlineId) && $this->airlineId != -1) {
		    $query .= "and om.airlineID IN ($this->airlineId) ";
		}
		...
		if ($this->tailsign != '') {
		    ...
		    $tailsignString = rtrim($tailsignString, ",");
		    $query .= "AND om.tailsignFound in ($tailsignString) ";
		}
...
// 		    $query .=" AND (om.uploadedTime between '$uploadStartDate' and '$uploadEndDate')";
		...
		$query .=" ORDER BY om.depAirport, om.arrAirport";
		
		
		error_log("Dep Airportlist Query: " . $query);
		$result = mysqli_query ( $this->dbConnection, $query );
	...
	private function getTailsignForAirlinePlatformConf() {
	...
	function getProcessedData($alId, $dbConnection, $mainDB, $request) {
		...
		error_log ( 'uploadEndDate: ' . $this->uploadEndDate );
		error_log ( 'airlineId...' . $this->airlineId );
		$query = "SELECT om.id, om.fileName, om.fileSize, om.status, om.tailsignInFile, om.tailsignFound as tailsign, om.flightNumber, om.depTime, om.arrTime, om.depAirport, om.arrAirport, om.failureReason, om.offloadDate, om.uploadedTime, om.remarks, om.flightLegIds, om.source, om.tailsignSource, ac.id as aircraftId";
		$query .= " FROM offloads_master om, aircrafts ac WHERE ";
		if ($this->airlineId != '') {
			if (strpos($this->airlineId, ',') !== false) {
			    $query .= "om.airlineId IN ($this->airlineId) AND ";
		...
		if (! $this->IsNullOrEmptyString ( $this->depStartDate ) and ! $this->IsNullOrEmptyString ( $this->depEndDate )) {
			$query .= " AND (DATE(om.depTime) between CAST('$this->depStartDate 'AS DATE) and CAST('$this->depEndDate 'AS DATE) )";
		}
		
		if (! $this->IsNullOrEmptyString ( $this->arrStartDate ) and ! $this->IsNullOrEmptyString ( $this->arrEndDate )) {
			$query .= " AND (DATE(om.arrTime) between CAST('$this->arrStartDate 'AS DATE) and CAST('$this->arrEndDate 'AS DATE))";
		}
		
		if (! $this->IsNullOrEmptyString ( $this->offloadStartDate ) and ! $this->IsNullOrEmptyString ( $this->offloadEndDate )) {
			$query .= " AND (DATE(om.offloadDate) between CAST('$this->offloadStartDate 'AS DATE) and CAST('$this->offloadEndDate 'AS DATE))";
		}		
		
		if (! $this->IsNullOrEmptyString ( $this->uploadStartDate ) and ! $this->IsNullOrEmptyString ( $this->uploadEndDate )) {
// 			$query .= " AND (DATE(om.uploadedTime) between CAST('$this->uploadStartDate 'AS DATE) and CAST('$this->uploadEndDate 'AS DATE))";
			$query .= " AND (om.uploadedTime between '$this->uploadStartDate 00:00:00' and '$this->uploadEndDate 23:59:59')";
		} else {
		...
		if (! $this->IsNullOrEmptyString ( $this->failureReason ) and ! $this->IsNullOrEmptyString ( $this->failureReason )) {
			$query .= " AND om.failureReason like '% $this->failureReason %'";
		}
		...
		if ($result and mysqli_num_rows ( $result ) > 0) {
			while ( $row = mysqli_fetch_assoc ( $result ) ) {
				if ($row ['status'] == "Processed" && empty ( $row ['flightNumber'] )) {
					$flightLegId = $this->getFirstToken ( $row ['flightLegIds'] );
	...
	function getRejectedData($alId, $dbConnection, $mainDB, $request) {
		...
		$query = "SELECT om.id, om.fileName, om.fileSize, om.status, om.tailsignInFile, om.tailsignFound as tailsign, om.flightNumber, om.depTime, om.arrTime, om.depAirport, om.arrAirport, om.failureReason, om.offloadDate, om.uploadedTime, om.remarks, om.flightLegIds, om.source, om.tailsignSource";
		$query .= " FROM offloads_master om WHERE ";
		if ($this->airlineId != '') {
// 			$query .= "om.airlineId=$this->airlineId AND ";
			if (strpos($this->airlineId, ',') !== false) {
			    $query .= "om.airlineId IN ($this->airlineId) AND ";
...
// 			$query .= " AND (DATE(om.uploadedTime) between CAST('$this->uploadStartDate 'AS DATE) and CAST('$this->uploadEndDate 'AS DATE))";
	...
	function getProcessedDataForExport($alId, $dbConnection, $mainDB, $request) {
		...
		$query = "SELECT fileName as FileName, fileSize as FileSize, status as Status, tailsignFound as TailSign, flightNumber as FlightNumber, depTime as DepartureTime, arrTime as ArrivalTime, depAirport as DepartureAirport, arrAirport as ArrivalAirport, offloadDate as OffloadDate, uploadedTime as UploadTime, failureReason as FailureReason, remarks as Remarks, source as Source, tailsignSource as TailsignSource";
		...
		if (! $this->IsNullOrEmptyString ( $depStartDate ) and ! $this->IsNullOrEmptyString ( $depEndDate )) {
			$query .= " AND (DATE(depTime) between CAST('$depStartDate 'AS DATE) and CAST('$depEndDate 'AS DATE))";
		}
		
		if (! $this->IsNullOrEmptyString ( $arrStartDate ) and ! $this->IsNullOrEmptyString ( $arrEndDate )) {
			$query .= " AND (DATE(arrTime) between CAST('$arrStartDate 'AS DATE) and CAST('$arrEndDate 'AS DATE))";
		}
		
		if (! $this->IsNullOrEmptyString ( $offloadStartDate ) and ! $this->IsNullOrEmptyString ( $offloadEndDate )) {
			$query .= " AND (DATE(offloadDate) between CAST('$offloadStartDate 'AS DATE) and CAST('$offloadEndDate 'AS DATE))";
		}
		
		if (! $this->IsNullOrEmptyString ( $uploadStartDate ) and ! $this->IsNullOrEmptyString ( $uploadEndDate )) {
			$query .= " AND (DATE(uploadedTime) between CAST('$uploadStartDate 'AS DATE) and CAST('$uploadEndDate 'AS DATE))";
		} else {
