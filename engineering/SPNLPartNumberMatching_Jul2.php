...
function updateSWVersion($airlineId, $aircraftId, $idFlightLeg){
	...
	foreach($swConfigs as &$swConfig){
		...
		$LRU_PartNumber_Query = "SELECT hostName, partNumber FROM ".$databaseName.".BIT_confSw WHERE idFlightLeg=".$idFlightLeg." AND hostName IN ('".$LRU_HostName_string."');";
		$result = mysqli_query($GLOBALS['dbConnection'], $LRU_PartNumber_Query);
		if ($result && mysqli_num_rows ( $result ) > 0) {
			while($row=mysqli_fetch_assoc($result)){
				$BIT_LRU_PartNumbers[$row['hostName']][] = $row['partNumber'];
			}
		}else{
			updateProcess_log("SW Config not found in BIT_confSw for: ".join(",", $LRU_Hostnames), $idFlightLeg, $databaseName);
			return false;
		}
