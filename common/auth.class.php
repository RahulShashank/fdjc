...
class Auth
{
	...
	*/
	...
	public function register($email, $password, $repeatpassword, $airlineids, $role)
	{
		$return['error'] = true;
		...
		if ($role == "") {
			$return['message'] = "Must choose a role";
			return $return;
		}
		
		if ($airlineids == '0, ' && $role != "Admin") {
			$return['message'] = "Must choose an airline";
			return $return;
		}

		$addUser = $this->addUser($email, $password, $airlineids, $role);
	...
	*/
	...
	/*
	* Checks to see if UID has permission to view page based on airline and role
	* @param string $hash
	* @param string $airline code
	* @return boolean
	*/
	
	public function checkAirline($user, $airlinecode)
	{
		$userAirlines = array($this->fromStringArray($user['approvedAirlines']));
		if ($user) {
			if (in_array($airlinecode, $userAirlines) || $airlinecode == 0) {
				return true;
			} else {
				return false;
			}
	...
	public function checkRole($user, $roles)
	{
		if ($roles[0] == "all") {
			return true;
		}
	...
	public function checkPermissionBool($hash, $roles, $airlinecode=0)
	{
		if(!isset($hash) || !$this->checkSession($hash)) {
			return false;
		} else {
	...
	public function checkPermission($hash, $roles, $airlinecode=0)
	{
		$checkPermissionBool = $this->checkPermissionBool($hash, $roles, $airlinecode);
	...
	private function updateSession($hash)	
	{
		$this->updateLastActiveTime($hash);
		$query = $this->dbh->prepare("SELECT id, uid, expiredate, ip, agent, cookie_crc FROM {$this->config->table_sessions} WHERE hash = ?");
		$query->execute(array($hash));
		...
		$uid = $row['uid'];
		$expiredate = date("Y-m-d H:i:s", strtotime($row['expiredate']));
		$updateexpiredate = date("Y-m-d H:i:s", strtotime($this->config->cookie_forget));
		
		$updateQuery = $this->dbh->prepare("UPDATE {$this->config->table_sessions} SET expiredate = ? WHERE hash = ?");
		if ($updateexpiredate > $expiredate) {
			$updateQuery->execute(array($updateexpiredate, $hash));
			setcookie($this->config->cookie_name, $hash, strtotime($updateexpiredate), $this->config->cookie_path);
			return true;
		}
	}
	
	private function updateLastActiveTime($hash)
	{
		$uid = $this->getSessionUID($hash);
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET lastActive = ? WHERE id = ?");
		$timeNow = date("Y-m-d H:i:s", time());
		$query->execute(array($timeNow, $uid));
	}
	
	/*
	* Retrieves name of airline from code
	* @param string $airlinecode
	* @return string airlinename
	*/
	
	public function getAirlinesIDs($hash) {
		$uid = $this->getSessionUID($hash);
		$user = $this->getUser($uid);
		$userAirline = $user['approvedAirlines'];
		return explode(", ", $userAirline);;
	}
	
	public function toStringArray($airlines) {
		$arrayString ="";
		foreach($airlines as $airline) {
			$arrayString = $arrayString . $airline . ', ';
		}
		return $arrayString;
	}
	
	public function fromStringArray($airlines) {
		$numbers = [];
		$tok = strtok($airlines, ', ');
		while ($tok != false) {
			array_push($numbers, intval($tok));
			$tok = strtok(', ');
		}
		return $numbers;
	}
	
	public function getAirlines($hash) {
		$uid = $this->getSessionUID($hash);
		$user = $this->getUser($uid);
		$userAirlines = $user['approvedAirlines'];
		$unserializeAirlines = $this->fromStringArray($user['approvedAirlines']);
		return $unserializeAirlines;
	}
	
	public function getAirlineID($hash) {
		$user = $this->getUser($hash);
	}
	
	public function getHomepage($hash)
	{
		$uid = $this->getSessionUID($hash);
		$user = $this->getUser($uid);
		$airline = $user['airlineid'];
		switch($user['role']) {
			case "Admin":
				return "admin/";
			case "Manager":
	...
	*/
	...
	public function updateUser($uid, $email, $airlines, $role, $isactive)
	{
		$return ['error'] = true;
		
		$user = $this->getUser($uid);
		$currentEmail = $user['email'];
		if ($email != $currentEmail && $this->isEmailTaken($email)) {
			$return['message'] = $this->lang["email_taken"];
			return $return;
		}
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, approvedAirlines = ?, role = ?, isactive = ?  WHERE id = ?");

		if(!$query->execute(array($email, $airlines, $role, $isactive, $uid))) {
			
			$return['message'] = $this->lang["system_error"] . " #16"; //Error code 16: Failed to update user. 
			return $return;
		}

		$return['error'] = false;
		$return['oldEmail'] = $currentEmail;
	...
	*/
	...
	private function addUser($email, $password, $airlineids, $role)
	{
		$return['error'] = true;
		...
		//Chris: Taking out email functionality. 
		/*
		...
		*/
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, password = ?, airlineid = ?, role = ?, approvedAirlines = ? WHERE id = ?");
		if(!$query->execute(array($email, $password, 1, $role, $airlineids, $uid))) {
			$query = $this->dbh->prepare("DELETE FROM {$this->config->table_users} WHERE id = ?");
			$query->execute(array($uid));
			$return['message'] = $airlineids;
	...
	*/
	...
	{
		$query = $this->dbh->prepare("SELECT email, password, airlineid, approvedAirlines, role, isactive FROM {$this->config->table_users} WHERE id = ?");
		$query->execute(array($uid));
	...
	*/
	...
	private function addRequest($uid, $email, $type)
	{
		...
		if($type == "activation") {
			...
			//Chris: Taking out confirmation email functionality
	...
	*/
	...
	public function resendActivation($email)
	{
		...
		//Chris: Removing email functionality. 
		/*
	...
	*/
	...
	public function changePassword($uid, $currpass, $newpass, $repeatnewpass, $isAdmin=0)
	{
		$return['error'] = true;
