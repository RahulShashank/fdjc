<?php
	...
	function getlruNameListData($lruNameSelected,$lruNameListData)
	{
		$index = array_search($lruNameSelected, $lruNameListData);
		if ( $index !== false ) {
			unset( $lruNameListData[$index] );
		}
		return $lruNameListData;
	}
	...
	function getLruName($lruType, $lruSubType) {
		...
		else if ($lruType == 26 && $lruSubType == 2) {
			$lruName = 'QSEB';
		}
		else if ($lruType == 31 && $lruSubType == 1) {
			$lruName = 'SDB';
		}
		else {
			$lruName = "Unknown LRU - type: $lruType / subtype: $lruSubType";
		}
